...
class SmallUNet(nn.Module):
    ...
    def forward(self, x):
        # Encoder
        x = self.dec1(self.enc2(self.enc1(x)))
        return torch.sigmoid(x)  # [B, out_channels, H, W]
...
class conv_block_nested(nn.Module):
    ...
    def __init__(self, in_ch, mid_ch, out_ch):
        ...
        self.conv3 = nn.Conv2d(out_ch, 1, kernel_size=1)
...
class PiDiNet(nn.Module):
    ...
    def get_weights(self):
        ...
        for pname, p in self.named_parameters():
            ...
            if "blur" in pname:
              continue            
            elif 'thinner' in pname:
                thinner_weights.append(p)
            elif 'bn' in pname:
                bn_weights.append(p)
            elif 'relu' in pname:
        ...
        return conv_weights, bn_weights, relu_weights, thinner_weights

    def gaussian_blur(self, x, kernel_size=5, sigma=1.0):
        # Create a Gaussian kernel
        def gaussian_kernel1d(size, std):
            coords = torch.arange(size).float() - size // 2
            kernel = torch.exp(-0.5 * (coords / std).pow(2))
            kernel /= kernel.sum()  # Normalize
            return kernel
    
        kernel_1d = gaussian_kernel1d(kernel_size, sigma)
        kernel_2d = kernel_1d[:, None] * kernel_1d[None, :]
        kernel_2d = kernel_2d.view(1, 1, kernel_size, kernel_size)  # Shape (1, 1, k, k)
    ...
    def forward(self, x,epoch):
        ...
        thin_out = self.thinner(outputs[-1])
        end_time = time.time()

        return outputs, thin_out, end_time - start_time
